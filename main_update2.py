...
def extension_for_url(country):
    df = pd.read_csv("CountryNameDomain.csv")
    key =[]
    for i in df['Country']:
...
def filter_sublinks(sublink_list,main_extension):
    df = pd.read_excel("Regulator_CyberSecurity_Keywords.xlsx", sheet_name='keywords')
...
with open(text_dir,'a') as document:
   ...
   # subflder='CyberSecurity'
    try:
        ...
        if not isExistmain:
            os.mkdir(path1) 
        #path2 = f'C:/Users/admin/Desktop/INT-INFOWAREHOUSE/CyberSecurityDeployment/{main_flder}/{subflder}'
        #isExistsub = os.path.exists(path2)
        #if not isExistsub:
        #    os.mkdir(path2)  
        path3 = f'{main_flder}\\policy'
        isExistsub_policy = os.path.exists(path3)
        if not isExistsub_policy:
            os.mkdir(path3)
        path4 = f'{main_flder}\\strategy'
        isExistsub_strategy = os.path.exists(path4)
        if not isExistsub_strategy:
            os.mkdir(path4)
        path5 = f'{main_flder}\\guildlines'
